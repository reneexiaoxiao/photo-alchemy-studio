...
本库工作流扩展到 **59 个**：31 个由 Renee 编写（21 个原创实现、3 个参考用户截图的独立再设计、7 个参考用户文字方向的独立实现）+ 28 个社区来源。其中 **9 个社区模块**随公开库附带，其余保留来源；目录显示、样图和验证进度以图鉴各条目的实际状态为准。2026-09-14 按免费个人非商业图鉴的用途恢复7张已获许可图片，并用4张 Renee 新生成样例补齐其余条目。社区风格与样例制作者分别署名；本地完整收藏仍保留原参考图。详见 [本次图片与许可更新](docs/public-previews-2026-09-14.md)。
...
2026-09-15 新增七种工作流：中式留白拼贴、邮票时刻、剪纸叙事、冰箱贴、悬衡小剧场、折景明信片、画面谜语，图鉴默认按「最近收录」排序。七项均有原图对照，冰箱贴有三类输入试作；底图来源、署名与限制见 [本次试作记录](docs/playful-examples.md)。
...
2026-09-21 新增写意墨趣双联、折纸造物双联、光窗剪影、贝壳矿物像素双联、新几何扁平、留白实物海报和手绘平涂；旅行记忆拼贴与剪纸拼贴复用已有入口，可用这些名称直接搜索。[方向对应、署名与样例限制](docs/september-additions.md)。

2026-09-21 再增通透釉面马赛克与5个冰箱贴方向：金线珐琅、透光树脂、流沙夹层、珠母贝镶嵌、镂空浮影。旧版示例全部保留。[新入口、原图与样例限制](docs/luminous-keepsakes.md)。

## 开始使用

1. 下载仓库并打开 `index.html`，看样张、按视觉方向筛选、比较底图和结果，复制风格指令。照片题材、保留方式、来源和获取状态可组合筛选；「已附带指令」表示当前图鉴附有风格入口，不代表网页内直接生图。
...
新写的代码、31 个 Renee 编写工作流的新增文字和项目文档，在本项目可授权范围内采用 [MIT](LICENSE)。其中3个参考截图、7个参考文字方向的实现，不包含对截图、原提示词、原作者方法或第三方素材的重新授权。社区内容按对应目录中的原始许可证使用，根目录 MIT 不会覆盖第三方权利。作者、固定版本、许可和公开打包边界见 [THIRD_PARTY.md](THIRD_PARTY.md)。

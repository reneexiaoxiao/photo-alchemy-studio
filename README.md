...
目前有 **39 个风格条目**：11 个 Renee 原创工作流 + 28 个社区来源。其中9个社区模块随公开库附带，其余保留来源。公开图鉴显示28张效果图（11张原创样张 + 17张社区参考图）；另外11种放在底部推荐名单。Renee 本地完整收藏保留39张效果图。
...
有非商业限制、禁止再分发或未明确许可的原有来源只保留链接。公开版本与个人本地完整集合的内容范围因此不同。`assets/examples/` 与 `assets/transfer/` 是本项目生成的示例；可在本项目所能授权的权利范围内按 MIT 复用，不承诺生成内容在所有司法辖区均存在排他著作权。

...
let toastTimer;
let direction = 'all', destination = 'all';
...
function promptFor(s) {
  ...
  return `用 $photo-alchemy 的 ${s.id} 处理我附上的照片。${source}保留照片里的主要空间关系、标志性细节与物体数量，按照 ${s.label} 的工作流转译，优先适合分享的构图。未提供的地点、日期和文字不要编造。${extra}`;
}
function showImage(s) {
 const hasSource=Boolean(s.sourceImage);$('viewer-title').textContent=s.label+(hasSource?' · 原图对照':' · 参考效果');
 $('source-figure').hidden=!hasSource;$('compare').classList.toggle('single',!hasSource);
 if(hasSource)$('compare-source').src=s.sourceImage;
 $('compare-art').src=s.image;$('compare-art').alt=s.label+'参考效果';
 $('compare-label').textContent=({'UPSTREAM EXAMPLE':'作者公开参考图','LOCAL IMAGEGEN SAMPLE':'本地生成参考图'}[s.imageCaption]||s.imageCaption||'社区参考图');
 $('viewer-credit').textContent=s.origin==='original'?'工作流设计：Renee · AI 辅助':s.imageCredit||s.credit||s.source;
 $('viewer').showModal();
}
function matches(s) { const query=$('search').value.trim().toLowerCase(); return (!query || [s.id,s.label,s.summary,s.source,...(s.best||[]),...(s.destinations||[])].join(' ').toLowerCase().includes(query)) && (filter==='all'||(filter==='installed'?s.installed:s.origin===filter)) && (direction==='all'||s.direction===direction) && (destination==='all'||s.destinations?.includes(destination)) && (!chosen||chosen.has(s.id)); }
function render() {
 ...
 for(const s of visible){
  ...
  if(s.image){const art=button('',()=>showImage(s),'art-button');const img=document.createElement('img');img.src=s.image;img.alt=s.label+'演示样张';img.loading='lazy';art.append(img,textNode('span',s.origin==='original'?'Renee 原创':'社区参考','image-label'),textNode('span',s.sourceImage?'原图对照 ↗':'查看大图 ↗','image-action'));card.append(art);}
...
for(const id of ['direction','destination'])$(id).addEventListener('change',()=>{direction=$('direction').value;destination=$('destination').value;chosen=null;$('selection').hidden=true;render();});
$('clear').addEventListener('click',()=>{direction='all';destination='all';$('direction').value='all';$('destination').value='all';$('search').value='';chosen=null;filter='all';$('selection').hidden=true;document.querySelectorAll('[data-filter]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.filter==='all')));render();});
$('random').addEventListener('click',()=>{direction='all';destination='all';$('direction').value='all';$('destination').value='all';const pool=styles.filter(s=>s.installed&&!s.manualOnly);for(let i=pool.length-1;i>0;i--){const j=crypto.getRandomValues(new Uint32Array(1))[0]%(i+1);[pool[i],pool[j]]=[pool[j],pool[i]];}const sample=pool.slice(0,3);chosen=new Set(sample.map(s=>s.id));filter='all';$('search').value='';document.querySelectorAll('[data-filter]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.filter==='all')));$('selection').textContent='本次随机：'+sample.map(s=>s.label).join(' / ');$('selection').hidden=false;render();$('collection').scrollIntoView();});
$('auto').addEventListener('click',()=>copy('用 $photo-alchemy 看我附上的旅行照，结合主体、空间层次、光影和保真要求，推荐三种差异明显的照片二创风格，每种给一个具体理由。先给选择，不生成图片；排除仅允许手动点选的来源。'));

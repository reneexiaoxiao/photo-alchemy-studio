...
let toastTimer;
let subject='all', preservation='all', origin='all', availability='all', sortBy='newest';
...
function showImage(s) {
 ...
 $('viewer-credit').textContent=workflowCredit(s);
 ...
 function selectExample(index){
  ...
  if(hasSource){$('compare-source').src=v.sourceImage;$('compare-source').alt=v.sourceCaption||'输入照片';$('source-caption').textContent=v.sourceCaption||'输入照片';if(safeLink(v.sourceUrl||''))$('source-caption').append(document.createTextNode(' · '),link('查看底图来源 ↗',v.sourceUrl));}
  $('viewer-credit').textContent=v.imageCredit||(workflowCredit(s));
...
for(const id of ['subject','preservation','origin','availability','sort'])$(id).addEventListener('change',()=>{subject=$('subject').value;preservation=$('preservation').value;origin=$('origin').value;availability=$('availability').value;sortBy=$('sort').value;chosen=null;$('selection').hidden=true;render();});
